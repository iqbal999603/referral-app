...
# Header
st.markdown('<div class="main-header"><h1>📱 Ali Mobiles Repairing 03006762827 Ali Laal Road Layyah</h1><p>ریفرل کریں اور موبائل ریپئرنگ میں ڈسکاؤنٹ حاصل کریں</p></div>', unsafe_allow_html=True)
            ...
            if not name or not mobile or not password:
                ...
                if c.fetchone():
                    ...
                    if ref_code:
                        ...
                        referrer = c.fetchone()
                        if referrer:
                            c.execute("UPDATE users SET points = points + 10 WHERE referral_code=?", (ref_code,))
                            conn.commit()
                            st.success("🎉 آپ کے ریفرر کو 10 پوائنٹس مل گئے۔")
                        else:
